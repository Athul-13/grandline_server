import { RefundConfirmationEmailData } from '../../types/email.types';
...
/**
 * Renders HTML version of refund confirmation email
 * Styled with GRANDLINE theme colors
 */
export function renderRefundConfirmationHTML(data: RefundConfirmationEmailData): string {
...
<head>
  ...
  <title>Refund Confirmation - GRANDLINE</title>
</head>
        ...
        <table role="presentation" style="max-width: 600px; width: 100%; background-color: #FFFFFF; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #C5630C 0%, #b5590b 100%); padding: 40px 30px; text-align: center;">
              <h1 style="color: #FFFFFF; margin: 0; font-size: 28px; font-weight: 600;">Refund Processed</h1>
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="font-size: 16px; color: #1a1a1a; margin: 0 0 30px 0; line-height: 1.8;">
                We have processed a ${refundType.toLowerCase()} for your reservation. The refunded amount will be credited back to your original payment method within 5-10 business days.
              </p>
              ...
              <div style="background-color: #F4F1DE; border: 2px solid #E8E5D0; border-radius: 8px; padding: 25px; margin: 30px 0;">
                ...
                <table role="presentation" style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr>
                    ...
                    <td style="padding: 8px 0; color: #1a1a1a; font-size: 14px; font-weight: 600;">${data.reason}</td>
                  </tr>
                  ` : ''}
                </table>
              </div>
...
/**
 * Renders plain text version of refund confirmation email
 */
export function renderRefundConfirmationText(data: RefundConfirmationEmailData): string {
  const greeting = data.fullName ? `Hello ${data.fullName},` : 'Hello,';
  const tripName = data.tripName || 'Your Trip';
  const tripTypeLabel = data.tripType === 'one_way' ? 'One Way' : 'Round Trip';
  const refundType = data.isFullRefund ? 'Full Refund' : 'Partial Refund';
